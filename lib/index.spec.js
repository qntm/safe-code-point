...
const eaw8 = eastAsianWidth('8.0')
const eaw9 = eastAsianWidth('9.0')
const eaw10 = eastAsianWidth('10.0')
const ccc8 = canonicalCombiningClass('8.0')
const ccc10 = canonicalCombiningClass('10.0')
const gc9 = generalCategory('9.0')
const np10 = normalizationProperties('10.0')
...
describe('safe-code-point', () => {
  describe('canonicalCombiningClass', () => {
    it('works', () => {
      expect(ccc8(0)).toBe(0)
      expect(ccc8(30)).toBe(0)
      expect(ccc8(parseInt('0345', 16))).toBe(240)
    })
  })
  ...
  describe('eastAsianWidth', () => {
    it('works', () => {
      expect(eaw8(0)).toBe('N')
      expect(eaw8(0x001F)).toBe('N')
      expect(eaw8(0x0020)).toBe('Na')

      expect(eaw9(0x00F8)).toBe('A')
      expect(() => eaw9(0xABFF)).toThrow()

      expect(() => eaw10(0xE1000)).toThrow()
      expect(eaw10(0x30000)).toBe('W')
    })
  })

  describe('generalCategory', () => {
    it('works', () => {
      expect(gc9(0)).toBe('Cc')
      expect(gc9(31)).toBe('Cc')
      expect(gc9(parseInt('055A', 16))).toBe('Po')
    })
  })

  describe('normalizationProperties', () => {
    it('works', () => {
      expect(np10(parseInt('037A', 16), 'FC_NFKC')).toBe('0020 03B9')
      expect(np10(parseInt('E0002', 16), 'NFKC_CF')).toBe('')
      expect(np10(parseInt('FB1D', 16), 'NFC_QC')).toBe('N')
      expect(np10(parseInt('10FFFF', 16), 'NFKD_QC')).toBe('Y')
    })
  })
    ...
    it('works', () => {
      ...
      allBlockStarts.forEach(blockStart => {
        for (let i = 0; i < 1 << 8; i++) {
          ...
          expect(eaw8(codePoint)).toBe(isInNeutralBlock ? 'N' : 'W')
        }
      })
    })
  })
