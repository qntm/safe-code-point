...
export default async (version, options) => {
  ...
  options = options || {};
  const disallowedSet = new Set(options.disallowed);
  const safeCategories = options.safeCategories || gcIsSafe;

  const safeCodePoint = codePoint => {
    const passesQuickChecks = quickChecks.every(property =>
      np(codePoint, property) === 'Y'
    )
    const inSafeGc = safeCategories[gc(codePoint)] === true
    const hasCcc0 = ccc(codePoint) === 0

    return passesQuickChecks && inSafeGc && hasCcc0 && !disallowedSet.has(codePoint)
  }
