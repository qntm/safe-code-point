...
describe('safe-code-point', () => {
  ...
  describe('SafeCodePoint', () => {
    it('rejects on a bad Unicode version', async () => {
      await assert.rejects(SafeCodePoint('0.0.0'), Error('404 while fetching http://www.unicode.org/Public/0.0.0/ucd/extracted/DerivedCombiningClass.txt'))
    })
  })

  describe('canonicalCombiningClass', () => {
    it('works', () => {
      assert.throws(() => scp8.canonicalCombiningClass(-1), Error('Canonical_Combining_Class unknown for -1'))
  ...
  describe('generalCategory', () => {
    it('works', () => {
      assert.throws(() => scp9.generalCategory(-1), Error('General_Category unknown for -1'))
  ...
  describe('normalizationProperties', () => {
    it('works', () => {
      assert.throws(() => scp10.normalizationProperties(-1, 'NFC_QC'), Error('Unrecognised code point: -1'))
      assert.throws(() => scp10.normalizationProperties(parseInt('037A', 16), 'AAAAA'), Error('Unrecognised property: AAAAA'))
